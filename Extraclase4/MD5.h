...
// MD5 operations
    ...
    constexpr uint32_t stepHelper(uint32_t fun_val, uint32_t s, uint32_t b) {
        return ((fun_val << s) | ((fun_val & 0xffffffff) >> (32 - s))) + b;
    }

// Aplicación genérica de la función "fun"

    template<typename Functor>
    constexpr uint32_t myStep(Functor fun, uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t x, uint32_t t, uint32_t s) {
        return stepHelper(a + fun(b, c, d) + x + t, s, b);
    }

// Recuperar el n-ésimo uint32_t en el búfer
    ...
    /**Generadores de búfer iniciales**/

    // index_tuples para llenar el búfer inicial
    ...
    /** Función para crear el búfer.
    *
    * Para índices < longitud de cadena: muestra el i-ésimo carácter de la cadena.
    * Para índices> longitud de cadena: salida 0.
    * Si índice == longitud de cadena: salida 0x80
    * Si índice == 56: longitud de la cadena de salida << 3
    *
    */
    ...
    /**
    * Implementación de matriz simple para el acceso constexpr a sus elementos.
    */
...
/**Implementación del MD5**/

    typedef std::array<char, 16> md5_type;

    /**
    * Hay 64 pasos. El i-ésimo paso tiene la misma estructura que el i-ésimo + 4 escalones.
    * Eso significa que podemos repetir la misma estructura y elegir la adecuada
    * constantes y función a aplicar, según el número de paso.
    */
    ...
    /**
    *Funcion para convertir el resultado MD5 (que es 4 uint32_t), a una std :: array <char, 16>.
    */
    ...
    struct md5_step<n, 3> {
        static constexpr md5_type do_step(const char *data, uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
            return md5_step<n + 1, (n + 1) % 4>::do_step(data, a, myStep(md5_functions[n / 16], b, c, d, a,
    ...
    struct md5_step<n, 2> {
        static constexpr md5_type do_step(const char *data, uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
            return md5_step<n + 1, (n + 1) % 4>::do_step(data, a, b, myStep(md5_functions[n / 16], c, d, a, b,
                                                                            data32(data, md5_indexes[n]),
    ...
    struct md5_step<n, 1> {
        static constexpr md5_type do_step(const char *data, uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
            return md5_step<n + 1, (n + 1) % 4>::do_step(data, a, b, c, myStep(md5_functions[n / 16], d, a, b, c,
                                                                               data32(data, md5_indexes[n]),
    ...
    struct md5_step<n, 0> {
        static constexpr md5_type do_step(const char *data, uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
            return md5_step<n + 1, (n + 1) % 4>::do_step(data, myStep(md5_functions[n / 16], a, b, c, d,
                                                                      data32(data, md5_indexes[n]), md5_constants[n],
                                                                      md5_shift[n]), b, c, d);
        }
    };
